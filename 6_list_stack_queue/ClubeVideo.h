...
public:
	...
	string imprimirFilmes() const; //prints all movies; rented and not rented
	list<string> titulosDisponiveis() const; //tittle list not repeated
...
class FilmeInexistente{
	const string title;
public:
	FilmeInexistente(string movie): title(movie){}
	string msg() const { return "Titulo Inexistente: " + title ;}
};
